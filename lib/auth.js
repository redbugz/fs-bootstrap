...
function validateSession(sessionId, userId, user) {
  uri = "https://api.familysearch.org/identity/v2/session?dataFormat=application/json&sessionId=" + sessionId;
...
module.exports = function(app, config){
  ...
  everyauth.familysearch.configure({
    developerKey:       process.env.FS_DEV_KEY,
  ...
  app.use(function(req, res, next) {
    ...
    var userId = req.session && req.session.auth ? req.session.auth.userId : undefined;
